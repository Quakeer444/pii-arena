...
const configurations = [
  ...
  { id: "fastino", name: "GLiNER2 Fastino", label: "Single detector", kind: "solo" },
  { id: "pplx+fastino", name: "Fastino + PPLX", label: "Two detectors", kind: "cuda" },
  { id: "pplx+fastino+mmbert", name: "Fastino + PPLX + mmBERT", label: "Three detectors", kind: "cuda" },
  { id: "pplx+fastino+bardsai", name: "Fastino + PPLX + BardsAI EU", label: "CPU + GPU", kind: "hybrid" },
  { id: "pplx+fastino+bardsai+mmbert", name: "Fastino + PPLX + BardsAI EU + mmBERT", label: "Four detectors", kind: "hybrid" },
] as const;
...
function Card({ title, description, children, tools, note, className = "" }: { title: string; description: string; children: ReactNode; tools?: ReactNode; note?: string; className?: string }) {
  return <article className={`decision-view ${className}`}>
    <div className="decision-view-head"><div><h3>{title}{note&&<Tooltip><TooltipTrigger asChild><button type="button" className="decision-info" aria-label={`About ${title}`}><CircleHelp/></button></TooltipTrigger><TooltipContent className="max-w-80 leading-relaxed">{note}</TooltipContent></Tooltip>}</h3><p>{description}</p></div>{tools}</div>
    {children}
  </article>;
}
...
function ModelName({ system, name, complete = true }: { system: string; name?: string; complete?: boolean }) {
  return <div className="decision-model-name">
    ...
    {!complete && <small className="coverage-note" title="Some datasets are missing or excluded because of training overlap. Compare only shared datasets.">Not all datasets</small>}
  </div>;
}
...
function Robustness({ rows }: { rows: RobustnessRow[] }) {
  ...
  return <Card title="Performance from easy to extreme" description="See how much sensitive data each detector hides completely at every difficulty level. Higher is better." note="Each value is the average across datasets in that level. Known training overlaps are removed; detectors covering all datasets are shown first." tools={<SearchBox value={query} onChange={setQuery} placeholder="Find a detector…" label="Search detectors by difficulty"/>}>
    <Scroll label="Detector results by difficulty" className="robustness-scroll">
      <table className="robustness-table"><thead><tr><th scope="col">Detector</th>{tiers.map(tier => <th scope="col" key={tier} className={`tier-${tier.toLowerCase()}`}><i className="difficulty-dot"/>{tier}</th>)}</tr></thead>
...
function Protection({ data, datasetIds }: { data: Benchmark; datasetIds: Set<string> }) {
  ...
  return <Card title="One detector vs several combined" description="See whether combining detectors hides more sensitive data and how much extra text it masks." note="These combinations merge saved predictions. Extra text masked is measured on rows without labels, not verified false positives." tools={<Link className="decision-text-link" href="/ensembles">See combinations<ArrowRight size={14}/></Link>} className="protection-card">
    ...
    <div className="protection-foot"><div className="decision-legend"><span><i className="legend-solo"/>Single detector</span><span><i className="legend-cuda"/>Runs on CUDA</span><span><i className="legend-hybrid"/>CPU + GPU</span></div></div>
  </Card>;
}
...
function Rankings({ rows, className = "" }: { rows: SliceRow[]; className?: string }) {
  return <div className={`slice-rankings ${className}`}>{rows.map(row => <div className="slice-row" key={row.system}>
    <ModelName system={row.system} name={row.name} complete={row.complete}/>
    <div className="slice-score mono" title={`${row.coverage.gold.toLocaleString("en-US")} labeled items`}>{percent(row.value, 1)}<div className="slice-track"><i style={{ width: `${row.value ?? 0}%` }}/></div></div>
  </div>)}</div>;
}
