import type { Benchmark, RecordRow } from "./benchmark.ts";
import { DIFFICULTY, MODEL_LABELS, type DifficultyTier } from "./decision-data.ts";
...
export type RobustnessRow = ModelResult &
  Record<DifficultyTier, number | null> & {
...
function coverage(rows: RecordRow[], datasetIds: Set<string>): Coverage {
  const eligible = rows.filter(
    (row) => datasetIds.has(row.dataset) && !row.train,
  );
  const present = new Set(eligible.map((row) => row.dataset));
  const excluded = new Set(
    rows
      .filter((row) => datasetIds.has(row.dataset) && row.train)
      .map((row) => row.dataset),
  );
  return {
...
function fullyHidden(rows: RecordRow[], macro: boolean): number | null {
  ...
  if (macro)
    return (
      eligible.reduce((sum, row) => sum + (100 * row.hidden) / row.gold, 0) /
      eligible.length
    );
  return (
    (100 * eligible.reduce((sum, row) => sum + row.hidden, 0)) /
    eligible.reduce((sum, row) => sum + row.gold, 0)
  );
}
...
export function buildDecisionMetrics(
  ...
  const datasetIds =
    scopeIds ?? new Set(data.datasets.map((dataset) => dataset.id));
  const tierIds = Object.fromEntries(
    tiers.map((tier) => [
      tier,
      new Set(
        DIFFICULTY.filter(
          (row) => row.tier === tier && datasetIds.has(row.dataset),
  ...
  const domainIds = {
    pii: new Set(
      data.datasets
        .filter(
          (dataset) => dataset.kind === "pii" && datasetIds.has(dataset.id),
        )
    ...
    secrets: new Set(
      data.datasets
        .filter(
          (dataset) => dataset.kind === "secrets" && datasetIds.has(dataset.id),
        )
        .map((dataset) => dataset.id),
    ),
  };
  const categoryIds = new Map(
    data.categories.map((category) => [category.id, new Set<string>()]),
  );
  ...
  const models = data.systems
    .filter((system) => system.kind === "model")
    .map((system) => {
    ...
    .filter((row) => row.fullyHidden !== null)
    .sort(
      (a, b) =>
    ...
    return models
      .map((model) => {
        const rows = model.rows
          .filter((row) => ids.has(row.dataset))
          .flatMap((row) => {
            if (!categoryId) return [row];
            const category = row.categories.find(
              (category) => category.id === categoryId && category.gold > 0,
            );
            return category
              ? [{ ...row, gold: category.gold, hidden: category.hidden }]
              : [];
          });
  ...
  return {
    robustness: models.map(({ rows, ...model }) => ({
      ...
      ...(Object.fromEntries(
        tiers.map((tier) => [
          tier,
          fullyHidden(
            rows.filter((row) => tierIds[tier].has(row.dataset)),
            true,
          ),
        ]),
      ) as Record<DifficultyTier, number | null>),
      coverage: Object.fromEntries(
        tiers.map((tier) => [tier, coverage(rows, tierIds[tier])]),
      ) as Record<DifficultyTier, Coverage>,
    })),
    domains: models.map(({ rows, ...model }) => ({
      ...model,
      pii: fullyHidden(
        rows.filter((row) => domainIds.pii.has(row.dataset)),
        false,
      ),
      secrets: fullyHidden(
        rows.filter((row) => domainIds.secrets.has(row.dataset)),
        false,
      ),
      coverage: {
    ...
    categories: Object.fromEntries(
      [...categoryIds].map(([categoryId, ids]) => [
        categoryId,
        slices(ids, categoryId),
      ]),
    ),
        ...
        slices(
          new Set(
            data.datasets
              .filter(
                (dataset) =>
                  datasetIds.has(dataset.id) && dataset.lang === language,
              )
              .map((dataset) => dataset.id),
          ),
        ),
